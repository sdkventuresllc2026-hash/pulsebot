  ...
  return {
    ...
    schema: {
      ...
      properties: {
        ...
        dealerCode: { type: ['string', 'null'] },
        dealerLabel: { type: ['string', 'null'] },
...
function normalizeExtraction(item, attachment = null) {
  return {
    ...
    dealerCode: clean(item.dealerCode),
    dealerLabel: clean(item.dealerLabel),
...
function mergeTfiberExtractions(items, { fallbackTmoOrderId = null } = {}) {
  ...
  const extracted = {
    ...
    dealerCode: pickUnique(normalizedItems, 'dealerCode', conflicts),
    dealerLabel: pickUnique(normalizedItems, 'dealerLabel', conflicts),
